...
jwt+threadlocal 可以在任意地方获取user信息

在用户登陆调用login接口时  通过返回给前端一个用户的token 之后前端每个接口都会在请求头加上这个token

后端拦截器拿到了这个token 进行解析 判断是否合法 是否过期等  如果通过验证 就把用户信息查询出来 放入静态的thradlocal , 之后就可以愉快的调用了。